...
#Code to be emulated
f_text = open('UNICORN_NACL_OPEN/text.bin', 'rb')
TEXT_SECTION = f_text.read()
f_text.close()

#Data section of program
f_data = open('UNICORN_NACL_OPEN/data.bin', 'rb')
DATA_SECTION = f_data.read()
f_data.close()

f_rodata = open('UNICORN_NACL_OPEN/rodata.bin', 'rb')
...
def node_method(signal_in):
	...
	decoded_message = mu.mem_read(0x1e3ac, 50)
