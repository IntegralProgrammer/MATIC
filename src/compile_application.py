...
Usage: python compile_application.py PROGRAM_GRAPH.json COMPILED_PROGRAM.json
...
"""
Maps the name of an Input Node to an ordered list of calls to be made to
Processing and Output Nodes so that the signal from the Input Node may
be properly propagated through the network.
"""
ORDER_OF_OPERATIONS = {}

for do_event in INPUT_NODES:
	...
	"""
	List of operations which must be completed for properly responding
	to an Input Node event
	"""
	operations_list = []
	...
	if do_event in INPUT_NODES:
		...
		"""
		The first operation that must be performed is the node_method()
		for the Input Node itself
		"""
		
		#Template for operation
		my_op = {"node_name" : None, "input_nodeports" : {"default" : None}}
		
		my_op['node_name'] = do_event
		operations_list.append(my_op)
		...
		while incomplete_returns_table:
			...
			for rn in node_returns_table:
				...
				if len(preds) == 0:
					...
					"""
					If this node is directly connected to an Input Node
					then this node's node_method() may be called
					
					"""
					my_op = {"node_name" : rn, "input_nodeports" : {ff_port : do_event}}
					operations_list.append(my_op)
				...
				"""
				Add calling this node's node_method() to the list of
				operations associated with the triggering of a given
				input node.
				"""
				my_op = {"node_name" : rn, "input_nodeports" : forward_rn}
				operations_list.append(my_op)
		...
		"""
		Add the list of operations to the dictionary entry for this
		input node
		"""
		ORDER_OF_OPERATIONS[do_event] = operations_list
		
	else:
		print "ERROR: No input node matching " + do_event

#Pretty print the compiled program
for input_n in ORDER_OF_OPERATIONS:
	#Begin with a few blank lines
	for i in range(0,3):
		print ""
	print input_n
	print "======================"
	
	for op in ORDER_OF_OPERATIONS[input_n]:
		print op


#Save compiled program to file
f_output = open(COMPILED_PROGRAM, 'w')
f_output.write(json.dumps(ORDER_OF_OPERATIONS))
f_output.close()
